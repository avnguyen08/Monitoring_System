/*
  * Company: Secured Solutions Group
  * Author: Aaron Nguyen
  * Project: Current Monitoring System Ver A (7-Segment Display)
  * 5/4/2023 
  * Summary: Program for client company, Solid State Systems, using the Adafruit Feather Esp32-s3 and 7-segment display feather board to making a 2-in-1 amperage and time tracker for client. Uses an external ADC (ADS 1015 or 1115) to read differential
  * voltage across shunt to detect the amperage. Uses moving average filter, buffer for false readings, potentiometer for scaling, and dip switches to change shunt mode, 
...
const int truncation = 20;             // amount of samples truncated at the end part of the array storing sensor values.
const int SMOOTHING_WINDOW_SIZE = 100;  // amount of samples stored
...
CircularBuffer<float, AVE_SIZE> aveBuffer;                  //Queue that is used to take the nth last value from the average value readings
...
float peak_detector(CircularBuffer<float, SMOOTHING_WINDOW_SIZE> *Buffer) {
  float max = 0;
  for (int i = 0; i < (SMOOTHING_WINDOW_SIZE - truncation); ++i) {
    debug("Buffer Value[");
    debug(i);
    debug("]:");
    debugln((*Buffer)[i]);
...
void setup() {
  ...
  matrix.begin(0x70);
  ads.setGain(GAIN_FOUR);  // 2x gain   +/- 2.048V  1 bit = 1mV | // 4x gain   +/- 1.024V  1 bit = 0.5mV
  if (!ads.begin()) {
  ...
  ads.begin();
  matrix.print(1.002, 3);  //Version number. 1st digit DC or AC (1 DC, 2 AC). 2nd digit hardware version updates. 3rd and 4th are for software version updates
  matrix.writeDisplay();
...
void loop() {
  sensorVal = abs((float)ads.getLastConversionResults())/2;  // polling for sensor values to see when there is actually significant voltage (voltage above the threshold value)
  sensBuffer.push(sensorVal);  //store sensor values into a circular buffer of sensor values
  Serial.print("sensorVal:");
  Serial.println(sensorVal);
  startTime = millis();
  if (sensBuffer[0] > sens && sensBuffer[1] > sens && sensBuffer[2] > sens && sensBuffer[3] > sens && sensBuffer[4]) {
    while (1) {
      flag = 0;
      flag2 = 1;
      ...
      Serial.print("sensorVal:");
      Serial.println(sensorVal);
      if (sensBuffer[0] < sens && sensBuffer[1] < sens && sensBuffer[2] < sens && sensBuffer[3] < sens && sensBuffer[4]) { break; }
      endTime = millis();
  ...
  //peak detector
  if (windowBuffer.isFull()) {
    matrixVal = peak_detector(windowBufferptr);
    windowBuffer.clear();
  }
  //Error LED Print
