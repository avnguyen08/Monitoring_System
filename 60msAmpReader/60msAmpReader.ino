...
int window_count = 0;       //total count of sensor readings
...
void loop() {
  ...
  while (sensBuffer[0] > sens && sensBuffer[1] > sens && sensBuffer[2] > sens && sensBuffer[3] > sens && sensBuffer[4] > sens && sensBuffer[5] > sens && sensBuffer[6] > sens
    ...
    if (sensorVal > 2) {
      ...
      aveBuffer.push(_sampleAvg);//add sample average to array
      window_count++;
  ...
  if (startOn_dft || !flag) {
    ...
    startOn_dft = 0;
    count_per_sec = sens_count / displayTime;  // calculates counts per sec
      debug("Total sensor readings: ");
      debugln(sens_count);
      debug("Total window readings: ");
      debugln(window_count);
      debug("Display Time: ");
      debugln(displayTime);
      debug("Sensor readings/sec: ");
      debugln(count_per_sec);
      debug("Window readings/sec: ");
      debugln(window_count/displayTime);
      debugln("------------------------");
    sens_count = 0;
    window_count = 0;
  }
  ...
  for (int i = 0; i < aveBuffer.size(); ++i) {
    ...
    if (i == aveBuffer.size() - 1) {
      matrixVal = total_ave / ((float)(aveBuffer.size())/10);
      total_ave = 0;
    }
  }
  //Error LED Print
